...
function fuzz() {
	...
	if { [[ ! -f "$called_fn_dir/.${FUNCNAME[0]}" ]] || [[ $DIFF == true ]]; } && [[ $FUZZ == true ]] &&
		...
		if [[ -s "webs/webs_all.txt" ]]; then
			...
			if [[ $AXIOM != true ]]; then
				interlace -tL webs/webs_all.txt -threads ${INTERLACE_THREADS} -c "ffuf ${FFUF_FLAGS} -silent -t ${FFUF_THREADS} -rate ${FFUF_RATELIMIT} -H \"${HEADER}\" -w ${fuzz_wordlist} -maxtime ${FFUF_MAXTIME} -u _target_/FUZZ -o - -of json 2>/dev/null | jq -r 'try .results[] | \"\(.status) \(.length) \(.url)\"' | sort -k1 | anew -q _output_/_cleantarget_.txt" -o $dir/fuzzing 2>>"$LOGFILE" >/dev/null
